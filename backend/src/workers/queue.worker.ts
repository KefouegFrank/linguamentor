...
import { queueService, QUEUE_NAMES, redisConnection } from '../services/queue.service';
...
export const createAIProcessingWorker = () => {
  return queueService.createWorker(QUEUE_NAMES.AI_PROCESSING, async (job: BullJob) => {
    const { jobId, data } = job.data;
    const { fileId, operation, targetLanguage, webhookUrl, userId } = data;

    console.log(`Processing AI job ${jobId}: ${operation} for file ${fileId}`);
    ...
    try {
      ...
      if (operation === 'translate' || operation === 'transcribe') {
        ...
        const outputUrl = await s3Service.uploadFile(outputBuffer, outputKey, 'application/json');

        // Create output file record
        const outputFile = await prisma.file.create({
          data: {
            filename: `${file.filename}-${operation}.json`,
...
export const createFileProcessingWorker = () => {
  return queueService.createWorker(QUEUE_NAMES.FILE_PROCESSING, async (job: BullJob) => {
    const { jobId, data } = job.data;
    const { fileId, operation, parameters = {} } = data;

    console.log(`Processing file job ${jobId}: ${operation} for file ${fileId}`);
    ...
    try {
      ...
      switch (operation) {
        case 'convert':
          ...
          const convertedContent = 'This is converted content from the original file.';
          const convertedBuffer = Buffer.from(convertedContent);
          const convertedKey = `conversions/${fileId}-converted-${Date.now()}.txt`;
          
          await s3Service.uploadFile(convertedBuffer, convertedKey, 'text/plain');

          // Create output file record
          const convertedFile = await prisma.file.create({
            data: {
              filename: `${file.filename}-converted.txt`,
          ...
          result = {
            originalFormat: file.contentType,
        ...
        case 'compress':
          ...
          const compressedContent = fileBuffer.slice(0, Math.floor(fileBuffer.length * 0.7));
          const compressedKey = `compressed/${fileId}-compressed-${Date.now()}.zip`;
          
          await s3Service.uploadFile(compressedContent, compressedKey, 'application/zip');

          // Create output file record
          const compressedFile = await prisma.file.create({
            data: {
              filename: `${file.filename}-compressed.zip`,
          ...
          result = {
            ...
            compressionRatio: file.size > 0 ? (file.size - compressedContent.length) / file.size : 0,
            outputFileId,
          };
          break;
...
export const createEmailNotificationWorker = () => {
  return queueService.createWorker(QUEUE_NAMES.EMAIL_NOTIFICATIONS, async (job: BullJob) => {
    const { jobId, data } = job.data;
